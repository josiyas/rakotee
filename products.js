...
function displayProducts(filteredProducts) {
  ...
  setTimeout(() => { // Simulate loading delay for UX
    ...
    filteredProducts.forEach(product => {
      ...
      // Build Snipcart add-to-cart button attributes
      const itemId = `prod-${product.id}`;
      const itemUrl = (window.location.origin || 'https://rakotee.site') + '/products.html';
      const imageUrl = (product.images && product.images[0]) ? (product.images[0].startsWith('/') ? window.location.origin + product.images[0] : product.images[0]) : (window.location.origin + '/products/fallback.png');
  ...
  <img src="${product.images[0].startsWith('/') ? product.images[0] : '/' + product.images[0]}" alt="${product.name}" class="product-img" loading="lazy" onerror="this.onerror=null;this.src='/products/fallback.png';">
        ...
        <button class="snipcart-add-item btn add-to-cart" 
          data-item-id="${itemId}"
...
// Modal Add to Cart uses Snipcart API if available, otherwise fallback to old behavior
if (addToCartBtn) {
  addToCartBtn.addEventListener('click', async () => {
    ...
    // If Snipcart is loaded, use the API
    if (window.Snipcart && Snipcart.api && Snipcart.api.items) {
      Snipcart.api.items.add({
        id: `prod-${selectedProduct.id}`,
        name: selectedProduct.name,
        price: selectedProduct.price,
        url: (window.location.origin || 'https://rakotee.site') + '/products.html',
        image: (selectedProduct.images && selectedProduct.images[0]) ? ((selectedProduct.images[0].startsWith('/') ? window.location.origin + selectedProduct.images[0] : selectedProduct.images[0])) : (window.location.origin + '/products/fallback.png'),
        quantity,
        customFields: [
      ...
      Snipcart.api.session.openCart();
    } else {
      // fallback: original add-to-cart logic (if any)
      console.warn('Snipcart not loaded - fallback add to cart');
    }
  });
}
